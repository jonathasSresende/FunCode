...
window.onload = function() {
    ...
    if (usuarioData) {
      // Verifica qual fase o usuário completou
      const faseConcluida = usuarioData.fase_concluida || 1; // Fase 1 é a padrão
  
      // Liberar as fases com base no progresso
      liberarFases(faseConcluida);
    } else {
      console.error("Usuário não encontrado.");
    }
  };
  
  // Função para liberar as fases com base na fase concluída
  function liberarFases(faseConcluida) {
    for (let i = 1; i <= faseConcluida; i++) {
      const fase = document.getElementById(`fase-${i}`);
      if (fase) {
        fase.classList.remove('fase-bloqueada');
        fase.classList.add('fase-liberada');
      }
    }
  }